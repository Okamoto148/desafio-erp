...
O sistema possui regras automatizadas para a precificação de contratos de serviços. A principal diretriz aplicada é:

* **Desconto por Volume de Serviços:** Contratos que contenham **10 ou mais serviços vinculados** recebem automaticamente um **desconto de 10%** sobre o valor total do Preço Base Mensal. O cálculo é realizado na camada de serviço do backend antes da persistência e faturamento.
...
docker exec -it erp-app php artisan migrate:fresh --seed
```
---
...
## Execução dos Testes Automatizados (Docker)

A suíte de testes do backend foi isolada para rodar em um banco de dados SQLite em memória (`:memory:`), garantindo velocidade e integridade, sem afetar o seu banco de dados local de desenvolvimento.

Para rodar todos os testes automatizados da aplicação através do ambiente Docker, execute o seguinte comando no seu terminal:

```bash
docker exec -it erp-app php artisan test
```
